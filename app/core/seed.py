...
def _make_project(i: int, user_ids: list[int]) -> dict:
    ...
    description = _FAKER.paragraph(nb_sentences=3) if _FAKER else f"Description for project {i}"
    ...
    return {
        ...
        "created_at": datetime.now(),
        "updated_at": datetime.now(),
        "deleted_at": random.choice([None, datetime.now() - timedelta(days=random.randint(1, 100))]),
    }


def _make_project_member(project_id: int, user_ids: list[int], existing_members: set) -> Optional[dict]:
    """Generate fake project member data."""
    available_users = [uid for uid in user_ids if (project_id, uid) not in existing_members]
...
def _make_sprint(i: int, project_id: int) -> dict:
    ...
    return {
        ...
        "created_at": datetime.now(),
        "updated_at": datetime.now(),
        "deleted_at": random.choice([None, datetime.now() - timedelta(days=random.randint(1, 60))]),
    }


def _make_task(i: int, project_id: int, sprint_ids: list[int], user_ids: list[int]) -> dict:
    ...
    sprint_id = random.choice(sprint_ids) if random.random() < 0.7 and sprint_ids else None
...
def seed_all(
    ...
    try:
        if truncate:
            ...
            sess.execute(text("TRUNCATE TABLE project_history RESTART IDENTITY CASCADE"))
            ...
            sess.execute(text("TRUNCATE TABLE project_members RESTART IDENTITY CASCADE"))
        ...
        # Get user IDs
        user_ids = [row[0] for row in sess.execute(text("SELECT id FROM users")).fetchall()]
        ...
        # Get project IDs
        project_ids = [row[0] for row in sess.execute(text("SELECT id FROM projects")).fetchall()]
        print(f"Created {len(project_ids)} projects")
        
        # 3. Create Project Members
        print(f"Creating project members...")
        existing_members = set()
        all_members = []
        
        for project_id in project_ids:
            for _ in range(members_per_project):
                member_data = _make_project_member(project_id, user_ids, existing_members)
                if member_data:
        ...
        for i in range(0, len(all_tasks), batch_size):
            ...
            sess.commit()
            print(f"  Inserted {min(i + batch_size, len(all_tasks))}/{len(all_tasks)} tasks")
        
        task_ids = [row[0] for row in sess.execute(text("SELECT id FROM tasks")).fetchall()]
        print(f"Created {len(task_ids)} tasks")
        
        # 6. Create Project Histories
        print(f"Creating project histories...")
        all_project_histories = []
        for project_id in project_ids:
            for _ in range(histories_per_entity):
                all_project_histories.append(_make_project_history(project_id, user_ids))
        ...
        print(f"Creating sprint histories...")
        all_sprint_histories = []
        for sprint_ids in sprint_by_project.values():
            for sprint_id in sprint_ids:
                for _ in range(histories_per_entity):
                    all_sprint_histories.append(_make_sprint_history(sprint_id, user_ids))
        ...
        print(f"Creating task histories...")
        all_task_histories = []
        for task_id in task_ids[:min(10000, len(task_ids))]:  # Limit to avoid too much data
            for _ in range(histories_per_entity):
        ...
        for i in range(0, len(all_task_histories), batch_size):
            ...
            sess.commit()
            print(f"  Inserted {min(i + batch_size, len(all_task_histories))}/{len(all_task_histories)} task histories")
        ...
        print(f"  - Histories: {len(all_project_histories) + len(all_sprint_histories) + len(all_task_histories)}")
...
def main():
    ...
    parser.add_argument("--users", type=int, default=900, help="Number of users to create")
    parser.add_argument("--projects", type=int, default=500, help="Number of projects to create")
    parser.add_argument("--sprints-per-project", type=int, default=50, help="Sprints per project")
    parser.add_argument("--tasks-per-project", type=int, default=1000, help="Tasks per project")
    parser.add_argument("--members-per-project", type=int, default=100, help="Members per project")
    parser.add_argument("--histories", type=int, default=5, help="History records per entity")
